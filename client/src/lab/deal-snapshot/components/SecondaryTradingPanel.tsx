...
import { TrendDownIcon } from '../../issuance-components/icons';

interface SecondaryTradingPanelProps {
  mode: HydrationMode;
  runId: number;
  delay?: number;
}

export function SecondaryTradingPanel({ mode, runId, delay = 0 }: SecondaryTradingPanelProps) {
  const { atLeast } = useHydration({ mode, runId, delay });

  const last = secondarySeries[secondarySeries.length - 1];
  ...
  const stats: { label: string; value: string }[] = [
    ...
    { label: 'Current spread', value: `+${secondarySummary.currentSpread.toFixed(1)} bps` },
    { label: 'Spread move', value: `${spreadChange.toFixed(1)} bps` },
        ...
        {atLeast('data') ? (
          <div className="lab-fade-in h-full w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={secondarySeries} margin={{ top: 8, right: 8, left: -12, bottom: 0 }}>
                <defs>
                  <linearGradient id="ds-secondary-fill" x1="0" y1="0" x2="0" y2="1">
