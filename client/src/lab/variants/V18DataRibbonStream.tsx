...
function ribbonLabel(line: string) {
  ...
  try {
    ...
    const value =
      ...
      record.elapsedMs ??
      '';
    return value ? `${event}: ${String(value)}` : event;
  } catch {
    return raw;
  }
}

function DataRibbonStream({
  active,
  text,
}: {
  active: boolean;
  ...
  return (
    <div className="v18-stream" aria-label="Mock streamed data">
      <div ref={ref} className="v18-stream__viewport">
        <div className="v18-stream__track">
          {lines.map((line, index) => (
            <span
              key={`${index}:${line}`}
              className={
                index === lastIndex
                  ? 'v18-stream__pill v18-stream__pill--current'
                  : 'v18-stream__pill'
              }
            >
              {ribbonLabel(line)}
              {active && index === lastIndex && (
                <span className="v18-stream__cursor" aria-hidden>
                  |
                </span>
              )}
            </span>
...
// V18 · Orthogonal V15 fork: the sub-stream becomes a narrow horizontal
// data ribbon. This keeps proof-of-work visible but reads more like live
// instrumentation than a log.
export function V18DataRibbonStream({
  message,
  status,
  isLastMessage,
}: VariantRenderProps) {
  ...
  return (
    <ProcessingWorkflow
      ...
      renderRowExtra={({ step }) => {
        ...
        return text ? (
          <DataRibbonStream active={step.state === 'running'} text={text} />
        ) : null;
      }}
    >
