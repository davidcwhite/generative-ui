...
// V11 · Collapsible workflow (fork of V10). Same per-step treatment —
// circle indicator, workflow headline, JSON-ish sub — while the
// stream is live. Once the assistant response is ready, the step list
// collapses upward with a grid-row animation; a one-line summary
// (N steps) appears and the reader focuses on the answer. Tap the
// summary to expand the full run again, minimal chrome.
export function V11TerminalCollapse({
  message,
  status,
  isLastMessage,
}: VariantRenderProps) {
  ...
  // Collapse the run only after the stream has finished (or errored) so
  // the reader never fight with an intermediate flash.
  const canCollapse =
    lines.length > 0 &&
