...
function AmbientStream({
  ...
  return (
    <div className="v15-stream" aria-label="Mock streamed data">
      <div ref={ref} className="v15-stream__viewport">
        <div className="v15-stream__stack">
          {lines.map((line, index) => (
            <div
              key={`${index}:${line}`}
              className={
                index === lastIndex
                  ? 'v15-stream__line v15-stream__line--current'
                  : 'v15-stream__line'
              }
            >
              {line}
              {active && index === lastIndex && (
                <span className="v15-stream__cursor" aria-hidden>
                  |
                </span>
              )}
            </div>
...
// V15 · Fork of V14 that removes the literal "receiving" label. The
// stream sits as a quiet ambient layer beneath the stage, with a cleaner
// expandable detail surface for the full payload.
export function V15AmbientStream({
  message,
  status,
  isLastMessage,
}: VariantRenderProps) {
